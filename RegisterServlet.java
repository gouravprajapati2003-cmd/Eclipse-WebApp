...
public class RegisterServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (password == null || !password.matches("^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@#$%^&+=!]).{8,}$")) {
			response.sendRedirect("register.html?error=Password must contain uppercase, lowercase, number and special character");
			return;
		}

		// ---------------- SAVE DATA ----------------

		boolean dataSaveStatus = DatabaseConnection.insertUserData(
				firstName,
				lastName,
				Long.parseLong(mobileNum),
				email,
				password
		);

		if (!dataSaveStatus) {
			System.out.println(AppLogger.ERROR_LOG("DATA SAVE FAILED"));
		...
		boolean OTPSentStatus = OTPService.sendVerificationLink(
				email,
				firstName + " " + lastName,
				token
		);
		...
		if (OTPSentStatus) {
			...
			response.sendRedirect("register.html?error=Failed to send verification link");
		}
	}
